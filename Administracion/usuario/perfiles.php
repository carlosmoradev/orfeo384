...
<?
	...
	if ($codigo && $dependencia)
		{
		$isql = "SELECT USUARIO.USUA_CODI, USUARIO.USUA_DOC,USUARIO.USUA_NOMB,USUARIO.USUA_NACIM,USUARIO.USUA_LOGIN, DEPENDENCIA.DEPE_NOMB, USUARIO.USUA_AT, USUARIO.USUA_PISO, USUARIO.USUA_EXT, USUARIO.USUA_EMAIL, USUARIO.USUA_EMAIL_1, USUARIO.USUA_EMAIL_2, DEPENDENCIA.DEPE_CODI from USUARIO, DEPENDENCIA WHERE DEPENDENCIA.DEPE_CODI = USUARIO.DEPE_CODI AND USUARIO.USUA_CODI = " .$codigo ." AND USUARIO.DEPE_CODI= ".$dependencia;
		$rs = $db->query($isql);		
		...
		{
		$isql = "SELECT USUARIO.USUA_CODI,USUARIO.USUA_DOC,USUARIO.USUA_NACIM,USUARIO.USUA_NOMB,DEPENDENCIA.DEPE_CODI,USUARIO.USUA_LOGIN, DEPENDENCIA.DEPE_NOMB, USUARIO.USUA_AT, USUARIO.USUA_PISO, USUARIO.USUA_EXT, USUARIO.USUA_EMAIL, USUARIO.USUA_EMAIL_1, USUARIO.USUA_EMAIL_2 from USUARIO, DEPENDENCIA WHERE DEPENDENCIA.DEPE_CODI = USUARIO.DEPE_CODI AND USUARIO.USUA_DOC = " .$cedula;
		$rs = $db->query($isql);		
		$dependencia = $rs->fields["DEPE_CODI"];
		if ($rs->recordCount() > 0)
			{
...
<?
		...
		{
		$isql = "SELECT USUARIO.USUA_CODI, USUARIO.USUA_DOC,USUARIO.USUA_NACIM,USUARIO.USUA_NOMB,USUARIO.DEPE_CODI,USUARIO.USUA_LOGIN, USUARIO.USUA_AT, USUARIO.USUA_PISO, USUARIO.USUA_EXT, USUARIO.USUA_EMAIL, USUARIO.USUA_EMAIL_1, USUARIO.USUA_EMAIL_2 from USUARIO WHERE USUARIO.USUA_LOGIN = '" .$login ."'";
		$rs = $db->query($isql);		
		if ($rs->recordCount() > 0)
			{
			$valor_cedula = $rs->fields["USUA_DOC"];			
...
?>
    ...
    <tr>
      ...
      <td width="80%"><input name="email" type="text" size="73" maxlength="73" value='<?=$rs->fields["USUA_EMAIL"]?>'>
       <td width="50%" class="vbmenu_control" align="left"> <span class="Estilo5"><font size="2">E-Mail 2</font> </span></td>
       <td width="80%"><input name="email1" type="text" size="73" maxlength="73" value='<?=$rs->fields["USUA_EMAIL_1"]?>'>
       <td width="50%" class="vbmenu_control" align="left"> <span class="Estilo5"><font size="2">E-Mail 3</font> </span></td>
       <td width="80%"><input name="email2" type="text" size="73" maxlength="73" value='<?=$rs->fields["USUA_EMAIL_2"]?>'>
      </td>
    </tr>
    <tr>
