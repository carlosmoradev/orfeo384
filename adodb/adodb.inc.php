...
//	if (!defined('ADODB_DIR')) define('ADODB_DIR',dirname(__FILE__));
	...
	*/
	function create_trail($sql){
		...
		$opt = array("INSERT","UPDATE","DELETE");
		if(in_array($arrayQuery[0], $opt)){
			// OBTENEMOS LA IP REAL DEL USUARIO SALTANDO EL PROXY
			$realip = $this->get_ip_address();
			...
			if(strcmp($table,"SGD_AUDITORIA")!=0 && $type!=''){
				...
				$ipLocal = ""; //  $_SERVER["HTTP_X_FORWARDED_FOR"];
				 $query="INSERT INTO SGD_AUDITORIA (FECHA, USUA_DOC, IP, TIPO, TABLA, ISQL) VALUES ('".time()."', '".$_SESSION['usua_doc']."', '$realip - $ipLocal', '$type', '$table','".date("Y-m-d H:i:s")."> $sql')";
				 $this->Execute($query);
			}	
		}				 
	}
	...
	/**
	* Funcion para obtener la Ip de el cliente
	* Creditos: https://gist.github.com/cballou/2201933
	*/
